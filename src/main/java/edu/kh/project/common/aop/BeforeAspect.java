...
public class BeforeAspect {
	...
	// execution([접근제한자] 리턴타입(*) 클래스명(edu.kh.project..*Impl*) 메소드명*(..)) ([파라미터]))
//	@Before("execution(* edu.kh.project..*Impl*.*(..))")
	...
	public void beforeLog(JoinPoint jp) { // Advice(수행할 코드)
		...
		//					객체, 메서드, 파라미터 정보를 얻을 수 있게해주는 객체
		
		// 대상 객체의 간단한 클래스명(패키지명 제외)
		String className = jp.getTarget().getClass().getSimpleName();
		
		// 메서드 선언부( == 메서드 시그니처 / public void beforeLog )에서 메서드명만 얻어옴
		String methodName = jp.getSignature().getName();
		
		String str = "------------------------------------------\n";
		
		str += "[Start] : " + className + " - " + methodName + "()\n";
		// [Start] : BoardServiceImpl - selectBoardTypeList()
