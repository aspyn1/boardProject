...
// Intercepter : 요청/응답을 가로채는 객체
// Client <-> (Filter) <-> Dispatcher Servlet <-> (Interceptor) <-> Controller

// Filter : 전박적으로 쓰일 보안/인증/인가 관련 작업, 문자열 인코딩
// Interceptor : 요청에 대한 데이터 가공, 세부적으로 쓰일 보안/인증

public class BoardTypeInterceptor implements HandlerInterceptor{
	
	/* preHandle : 전처리					Despatcher Servlet -> Controller 사이
	 * postHandle : 후처리				Controller -> Dispatcher Servlet 사이
	 * afterCompletion : view 완성 후 	View Resolver -> Dispatcher Servlet 사이
	 */
