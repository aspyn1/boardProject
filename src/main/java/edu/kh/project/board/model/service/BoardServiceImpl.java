...
public class BoardServiceImpl implements BoardService {
	...
	public int like(Map<String, Integer> paramMap) {
		...
		// check == 0 / 1
		// check값이 무엇이냐에 따라서 BOARD_LIKE 테이블에 INSERT / DELETE 정해라
		// BOARD_LIKE 테이블 INSERT ( dao.insertBoardLike() ) 호출
		// BOARD_LIKE 테이블 DELETE ( dao.deleteBoardLike() ) 호출
	...
	public Map<String, Object> selectBoardList(Map<String, Object> paramMap, int cp) {
		...
		// 3. 특정 게시판에서 현재 페이지에 해당하는 부분에 대한 게시글 목록 조회
		// 단, 검색조건이 일치하는 글만
		// 어떤 게시판에서(boardCode) 몇페이지(pagination.currentPage)에 대해 
		// 몇개(pagination.limit)인지 조회
